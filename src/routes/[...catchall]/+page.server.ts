...
// catch-all gives those paths a route to match, redirecting into the user's last-used
// locale's (or `DEFAULT_LOCALE`'s, if none is persisted yet) dedicated `/404` route.
export const load: PageServerLoad = ({ cookies }) => {
	const locale = resolveFallbackLocale(cookies.get(LOCALE_COOKIE_NAME)) ?? DEFAULT_LOCALE;
	redirect(307, resolve(toPathname(`/${locale}/404`)));
};
